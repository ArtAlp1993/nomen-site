...
// ── Короткая ссылка через сервер (n8n на Railway) ────────────
// Разбор хранится в базе, в ссылке — только код (имя-slug + номер заказа):
//   https://nomen.website/r/anna-lee-k7q2m
// Требует онлайн-сервер: если он недоступен, короткая ссылка не откроется —
// поэтому длинная #r= остаётся самодостаточным запасным форматом (fallback в
// студии). Хост уже в CSP connect-src (app/layout.js). Content-Type НЕ ставим:
// строковое тело уходит как text/plain → браузер НЕ шлёт CORS-preflight (вебхук
// n8n не слушает OPTIONS), а ответ несёт Access-Control-Allow-Origin.
const READING_API = "https://primary-production-6abb7.up.railway.app/webhook";

// Сохранить карточку на сервере → { ok, code, url } (url — короткая ссылка).
export async function saveReadingLink(card, email = "") {
  const res = await fetch(`${READING_API}/nomen-reading-save`, {
    method: "POST",
    body: JSON.stringify({ card, email }),
  });
  if (!res.ok) throw new Error("save-failed");
  const data = await res.json();
  if (!data || !data.ok || !data.url) throw new Error((data && data.error) || "save-failed");
  return data;
}

// Прочитать карточку по коду короткой ссылки. Нет/битый код → Error("bad-link").
export async function fetchReadingByCode(code) {
  const res = await fetch(`${READING_API}/nomen-reading?c=${encodeURIComponent(code)}`);
  if (!res.ok) throw new Error("bad-link");
  const data = await res.json();
  if (!data || !data.ok || !data.card) throw new Error("bad-link");
  return data.card;
}

// Извлечь код короткой ссылки из location: путь /r/<code>, либо ?c= / #c=.
// Пусто → это не короткая ссылка (значит длинная #r=). Латиница/цифры/-/_.
export function extractReadingCode(loc = {}) {
  const qh = `${loc.search || ""} ${loc.hash || ""}`;
  const q = qh.match(/[?#&]c=([A-Za-z0-9_-]+)/);
  if (q) return q[1];
  const p = String(loc.pathname || "").match(/\/r\/([A-Za-z0-9_-]+)/);
  return p ? p[1] : "";
}
